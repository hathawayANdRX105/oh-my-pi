...
基线：fork/main `bbb6233916`    worktree：`.wt/session-daemon-2-host`    分支：`feat/session-daemon-host`
关联：无 issue。前置：阶段 0 判定「进入阶段 2」，且阶段 1 已合并或明确放弃。本阶段才是用户要的模型：一个进程付运行时，每个客户端只付自己的会话。

## 0 出发前：基线与禁区

- 基线：阶段 1 的合并提交；阶段 1 放弃时仍从 `bbb6233916` 起。任务书开工时把实际 SHA 填进本行，不许写「最新」。
- worktree：`git worktree add .wt/session-daemon-2-host -b feat/session-daemon-host <基线SHA>`。禁止在根工作树改文件。
- 图：worktree 内 `cg refresh`，graph commit 等于基线。
...
- `packages/coding-agent/src/cli/startup-cwd.ts`、`packages/coding-agent/src/main.ts`、`packages/coding-agent/src/modes/interactive-mode.ts`、`packages/coding-agent/src/slash-commands/builtin-lifecycle.ts`    改：去掉「这个进程的 cwd 就是会话 cwd」。`setProjectDir()` 只作用于 slot，不 `process.chdir()` 整个 host。
- `packages/coding-agent/src/config/settings.ts`    改：host 上的 `settings` 按 project + session 取，不再是模块级一份 Proxy 被所有会话写。
- `packages/coding-agent/src/registry/agent-registry.ts`    改：`AgentRegistry.global()` 不再等于「这个进程的唯一 Main」。host 里每个 slot 有自己的 registry。
...
`agent-session.ts` 有 469320 字节。默认不动它。只有某个方法直接读了 `process.cwd()`、`process.stdout` 或模块级 `settings` 时，才把那一次读取改成 slot 注入，并在 §2 逐条列出。禁止借机拆文件。
...
- 参考实现：
  - omp 已有的「一个进程多个 agent」是同一次调用里的 subagent：`registry/agent-registry.ts` `AgentRegistry.global()`，`registry/agent-lifecycle.ts` `park` / `revive`。这是 slot 的近亲，但它假定唯一的 `MAIN_AGENT_ID = "Main"`，不能直接当多客户端宿主。
  - 装配缝：`sdk.ts` `createAgentSession`。host 每 `attach` 一次调它，依赖从参数进，不从模块单例进。文件里已有注释：`preloadedExtensions` 不许跨 session 传递，因为它们关上了父会话的 cwd 和 eventBus。host 必须遵守。
...
- 调用面：改前必跑，再 grep 到行：
  - `cg callers setProjectDir`
  - `cg callers AgentRegistry.global`
  - `cg callers createAgentSession`
  - `git grep -n process.chdir packages/coding-agent/src`
  - `git grep -n 'from "../config/settings"' packages/coding-agent/src | head` 找出直接用模块级 `settings` 的文件
  每个调用者标改 / 不改 / 为什么。`eval/js/worker-core.ts` 里的 `process.chdir` 是 eval worker 自己的 cwd，不归 host，不改。
...
可以后做、本任务不派的两块：subagent 的 `AgentRegistry` 从「进程一个 Main」改成「slot 一个 Main」若超出上面列出的调用点，停下来写交接，不扩大。
