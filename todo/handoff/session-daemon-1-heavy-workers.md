...
已停止，不改代码。阶段 0 回执测得空会话 `MCP / embedder / browser` 可识别 self-size 仅 `0.13 MiB`，低于本阶段规定的 `30 MiB` 开工门槛；长会话为 `0.31 MiB`，增量 `0.18 MiB`。测量未预热这些资源，因此该数字不能证明真实峰值，但按本任务书的硬门槛仍必须停止，不能用推测绕过。阶段 2 同时因阶段 0 判定“只做阶段 1”而禁止启动。
